...
========
```
user          - Subject Index. An integer between 1 and 30 (inclusive) corresponding to the subject who participated in the study
activityName  - Name of the Activity for which the other values are shown. Valid Values are:
                WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING, STANDING, LAYING
```
Description of all other variables are present in the file [features_info.txt](https://github.com/tj---/dss-gcd/blob/master/features_info.txt)

The following transformations have been applied to the input dataset in order to generate the tidy dataset:
* Test and Training datasets are merged
* Variable names are read from features.txt, transformed (by removing '(',')','-' and adding '.') and applied as column names to the dataset.
* Columns that are identified as _mean_ or _standard deviation_ identified by the regex `mean|std` are selected by filtering.
* _Subjects_ and _ActivityNames_ are merged with the dataset.
* Mean is calculated for each variable per subjet per activity.
* The resulting data is dumped to a text file.
